...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default=ROOT / 'runs/train/exp47/weights/last.pt', help='initial weights path')    #预训练权重
    parser.add_argument('--cfg', type=str, default='models/hub/yolov5s-transformer.yaml', help='model.yaml path')    #指定模型配置文件路径
    parser.add_argument('--data', type=str, default=ROOT / 'data/PCB_ban.yaml', help='dataset.yaml path')    #数据集对应的参数文件
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch-low.yaml', help='hyperparameters path')
    parser.add_argument('--epochs', type=int, default=300)     #训练的轮数
    parser.add_argument('--batch-size', type=int, default=16, help='total batch size for all GPUs, -1 for autobatch')    #batchsize是批次大小
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=640, help='train, val image size (pixels)')  #模型在检测图片前先把图片resize成640*640的尺寸
